...
class CustomFeature(FeatureExtractor):
    ...
    def fit(self, text_set: list):
        ...
        for i in range(0, len(text_set)):
            for j in range(0, len(text_set[i])):
                padded_text = ["*"] * (self.n-1) + text_set[i]
                tuple_text = (*text_set[i][j:j+self.n], ) #unpacking the list into a tuple 
                if tuple_text not in self.ngram:
                    self.ngram[tuple_text] = index
                    index += 1
                else:
                    continue
    ...
    def transform(self, text):
        feature = np.zeros(len(self.ngram))
        padded_text = ["*"] * (self.n-1) + text
        for i in range(0, len(text)):
            tuple_text = (*padded_text[i:i+self.n],) #unpacking list into tuple
            if tuple_text in self.ngram:
                feature[self.ngram[tuple_text]] += 1
            else:
                feature[0] += 1
